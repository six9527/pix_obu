...
from autoware_perception_msgs.msg import LampState,TrafficLightState,TrafficLightStateArray,TrafficLight,Warning
...
class TcpInterfaceNode:
    def __init__(self):
        ...
        self.TrafficLight_bus_pub = rospy.Publisher("/light_color",TrafficLight,queue_size=10)
        ...
        self.warningType_pub= rospy.Publisher("/warning_type",Warning, queue_size=10)
...
#由于python2会将json传输的的数据解析为unicode，使用该函数将其转换为str
        ...
        if (len(dict_msg["id"])<=0):
            ...
            for i in range(len(dict_msg["lightDetails"])):
                if dict_msg["lightDetails"][i]["type"] == 2 and dict_msg["lightDetails"][i]["state"] != 3:
                    ...
                    self.TrafficLight_bus_pub.publish(TrafficLight_bus_msg)
    ...
    def reminder_push_in_alert_area(self, unicode_msg):
        ...
        warningInfo = dict_msg["warningInfo"].encode('utf-8')
        warningSuggestion = dict_msg["warningSuggestion"].encode('utf-8')
        distant = dict_msg["distant"]
        warning_pub.header.frame_id = "map"
        warning_pub.header.stamp = rospy.Time.now()
        warning_pub.warningType= warningType
        warning_pub.signType = signType
        warning_pub.evenType = evenType
        warning_pub.warningInfo = warningInfo
        warning_pub.warningSuggestion = warningSuggestion
        warning_pub.distant = distant
        ...
        print(warningInfo,warningSuggestion)
